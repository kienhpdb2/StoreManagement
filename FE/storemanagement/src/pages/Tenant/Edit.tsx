...
import EditTenantDto from './models/EditTenantDto';
import TenantDto from './models/TenantDto';
interface IOpenModalProps {
    open: boolean,
    closeForm: Function,
    curentTenant: TenantDto
}
const EditModal: React.FC<IOpenModalProps> = (props: IOpenModalProps) => {
    const { open, closeForm, curentTenant } = props;
    const onFinish = (input: EditTenantDto) => {
        axios.put(`Tenant/${curentTenant.id}`, input).then((res) => {
            let { data } = res;
            if (data.success) {
    ...
    return <>
        ...
        >
            <Form
                ...
                autoComplete="off"
                initialValues={{
                    ["name"]: curentTenant?.name,
                    ["description"]: curentTenant?.description,
                    ["address"]: curentTenant?.address,
                    ["storeOwnerName"]: curentTenant?.storeOwnerName,
                    ["email"]: curentTenant?.email,
                    ["phone"]: curentTenant?.phone
                }}
            >
